...
#define hairiness (9.5*(0.5 + 0.5*cos(0.1*time)) + 0.5)
#define fb(a) fb(a + snoise(a * hairiness + time * 0.1))
...
void main( void ) {
	...
	col.b = (0.6/col.b);
	col.r = 0.2*exp((0.5 + 0.3*sin((time*0.2)))*col.b + 0.2*cos(col.r/pow((2.*col.b),(0.5+0.5*cos(time*3.)))));
	
	col.rgb = (mix(0.3*col.brg,0.4*col.grb,mixer)) - vec3(0.3,0.1,0.3);
	
	gl_FragColor = vec4(col,1.0);//vec4(fb(vec3(position, (0.0)));
