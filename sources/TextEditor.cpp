...
void TextEditor::PageDown() {
	...
	if (nowCursor.col + maxLineNum < int(buffer.size()))
	{
		nowCursor.col += maxLineNum;
		lineOffset += maxLineNum;
	}
...
void TextEditor::MoveCursor(enum EditorCursorMoveType type) {
	...
	{
		case ECMT_DOWN:
			if (int(buffer.size()) > nowCursor.col + 1)
			{
				nowCursor.col++;
				if (int(buffer[nowCursor.col].length()) < nowCursor.row)
				{
		...
		case ECMT_LEFT:
			...
			break;
		case ECMT_RIGHT:
			if (int(buffer[nowCursor.col].length()) >= nowCursor.row + 1)
			{
				nowCursor.row++;
			}
			else
			{ // nowCursor.rowが右端
				if (nowCursor.col + 1 < int(buffer.size()))
				{
					nowCursor.col++;
					nowCursor.row = 0;
...
std::string TextEditor::ToString() {
	...
	{
		for (uint i = 0; i < log[logIndex].buffer.size(); i++)
...
TextEditorPtrBuffer TextEditor::GetVisibleText() {
	...
	if (logIndex == -1)
	{
		for (int i = lineOffset; i < lineOffset + maxLineNum && i < int(buffer.size()); i++)
		{
			buf.push_back(&buffer[i]);
		}
	}
	...
	{
		int offset = log[logIndex].lineOffset;
		for (int i = offset; i < offset + maxLineNum && i < int(log[logIndex].buffer.size()); i++)
		{
			buf.push_back(&log[logIndex].buffer[i]);
		}
	}
...
void TextEditor::Paste() {
	SnapShot();
	for (uint i = 0; i < copyBuffer.size(); i++)
	{
		if (i > 0)
			InsertCharacter('\n', false, false);
		for (uint j = 0; j < copyBuffer[i].length(); j++)
		{
...
void TextEditor::Delete() {
	...
	{
		if (nowCursor.col < int(buffer.size() - 1))
		{
...
void TextEditor::Home() {
	int v = 0;
	for (uint i = 0; i < buffer[nowCursor.col].length(); i++)
...
void TextEditor::InsertCharacter(char ch, bool historyEnable, bool autoIndent) {
	...
	if (ch == '\n')
	{
		...
		std::string b = "";
		if (buffer[nowCursor.col].length() >= (uint) nowCursor.row)
		...
		if (nowCursor.col > 0)
		{
			...
			int bracket = 0;
			bool pref = true;
			for (uint i = 0; i < buffer[nowCursor.col - 1].length(); i++)
			{
