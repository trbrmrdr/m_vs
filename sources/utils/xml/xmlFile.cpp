...
//---------------------------------------------------------
void xmlFile::setVerbose(bool _verbose){
}
...
//---------------------------------------------------------
bool xmlFile::loadFile(const string& fullXmlFile){

#ifdef TARGET_OF_ANDROID
    std::vector<char> buffer;
    readAsset(xmlFile, buffer);
    bool loadOkay = doc.ReadFromMemory(static_cast<char const *>(&buffer[0]), buffer.size());
#else
    bool loadOkay = doc.LoadFile(fullXmlFile);
#endif
    if (!loadOkay)
        LOGF("Couldn't load xml from \"%s\"", fullXmlFile.c_str());
...
//---------------------------------------------------------
void xmlFile::saveFile(const string& xmlFile){
...
//---------------------------------------------------------
void xmlFile::clearTagContents(const string& tag, int which){
...
//---------------------------------------------------------
void xmlFile::removeTag(const string& tag, int which){
...
//---------------------------------------------------------
int xmlFile::getValue(const string& tag, int defaultValue, int which){
...
//---------------------------------------------------------
double xmlFile::getValue(const string& tag, double defaultValue, int which){
    TiXmlHandle valHandle(NULL);
    if (readTag(tag, valHandle, which)){
...
//---------------------------------------------------------
string xmlFile::getValue(const string& tag, const string& defaultValue, int which){
    TiXmlHandle valHandle(NULL);
    if (readTag(tag, valHandle, which)){
...
//---------------------------------------------------------
bool xmlFile::readTag(const string&  tag, TiXmlHandle& valHandle, int which){
...
//---------------------------------------------------------
bool xmlFile::pushTag(const string&  tag, int which){
...
//---------------------------------------------------------
int xmlFile::getPushLevel(){
    return level;
}

//---------------------------------------------------------
bool xmlFile::tagExists(const string& tag, int which){
...
//---------------------------------------------------------
int xmlFile::getNumTags(const string&  tag){
...
//---------------------------------------------------------
int xmlFile::writeTag(const string&  tag, const string& valueStr, int which){
    ...
    for(uint x=0;x<tokens.size();x++){
        ...
        if ( !isRealHandle.ToNode() ||  addNewTag){
            ...
            for(uint i=tokens.size()-1;i>=x;i--){
                if (i == tokens.size()-1){
...
//---------------------------------------------------------
int xmlFile::setValue(const string& tag, int value, int which){
    char valueStr[255];
...
//---------------------------------------------------------
int xmlFile::setValue(const string& tag, double value, int which){
    char valueStr[255];
...
//---------------------------------------------------------
int xmlFile::setValue(const string& tag, const string& value, int which){
    int tagID = writeTag(tag, value, which) -1;
    return tagID;
}

//---------------------------------------------------------
int xmlFile::addValue(const string& tag, int value){
    char valueStr[255];
    sprintf(valueStr, "%i", value);
...
//---------------------------------------------------------
int xmlFile::addValue(const string&  tag, double value){
    char valueStr[255];
    sprintf(valueStr, "%f", value);
    int tagID = writeTag(tag, valueStr, -1) -1;
    return tagID;
...
//---------------------------------------------------------
int xmlFile::addValue(const string& tag, const string& value){
    int tagID = writeTag(tag, value, -1) -1;
    return tagID;
}

//---------------------------------------------------------
int xmlFile::addTag(const string& tag){
...
//---------------------------------------------------------
int xmlFile::addAttribute(const string& tag, const string& attribute, int value, int which){
    char valueStr[255];
    sprintf(valueStr, "%i", value);
...
//---------------------------------------------------------
int xmlFile::addAttribute(const string& tag, const string& attribute, int value){
    return addAttribute(tag,attribute,value,-1);
}

//---------------------------------------------------------
int xmlFile::addAttribute(const string& tag, const string& attribute, double value, int which){
    char valueStr[255];
...
//---------------------------------------------------------
int xmlFile::addAttribute(const string& tag, const string& attribute, double value){
    return addAttribute(tag,attribute,value,-1);
}

//---------------------------------------------------------
int xmlFile::addAttribute(const string& tag, const string& attribute, const string& value, int which){
    int tagID = writeAttribute(tag, attribute, value, which) -1;
    return tagID;
}

//---------------------------------------------------------
int xmlFile::addAttribute(const string& tag, const string& attribute, const string& value){
    return addAttribute(tag,attribute,value,-1);
}

//---------------------------------------------------------
void xmlFile::removeAttribute(const string& tag, const string& attribute, int which){
    vector<string> tokens = tokenize(tag,":");
    TiXmlHandle tagHandle = storedHandle;
    for (uint x = 0; x < tokens.size(); x++) {
        if (x == 0)
...
//---------------------------------------------------------
void xmlFile::clearTagAttributes(const string& tag, int which){
    vector<string> names;
...
//---------------------------------------------------------
int xmlFile::getNumAttributes(const string& tag, int which){
    vector<string> tokens = tokenize(tag,":");
    TiXmlHandle tagHandle = storedHandle;
    for (uint x = 0; x < tokens.size(); x++) {
        if (x == 0)
...
//---------------------------------------------------------
bool xmlFile::attributeExists(const string& tag, const string& attribute, int which){
    vector<string> tokens = tokenize(tag,":");
    TiXmlHandle tagHandle = storedHandle;
    for (uint x = 0; x < tokens.size(); x++) {
        if (x == 0)
...
//---------------------------------------------------------
bool xmlFile::getAttributeNames(const string& tag, vector<string>& outNames, int which){
    vector<string> tokens = tokenize(tag,":");
    TiXmlHandle tagHandle = storedHandle;
    for (uint x = 0; x < tokens.size(); x++) {
        if (x == 0)
...
//---------------------------------------------------------
int xmlFile::getAttribute(const string& tag, const string& attribute, int defaultValue, int which){
...
//---------------------------------------------------------
double xmlFile::getAttribute(const string& tag, const string& attribute, double defaultValue, int which){
...
//---------------------------------------------------------
string xmlFile::getAttribute(const string& tag, const string& attribute, const string& defaultValue, int which){
...
//---------------------------------------------------------
int xmlFile::setAttribute(const string& tag, const string& attribute, int value, int which){
    char valueStr[255];
    sprintf(valueStr, "%i", value);
    int tagID = writeAttribute(tag, attribute, valueStr, which) -1;
    return tagID;
...
//---------------------------------------------------------
int xmlFile::setAttribute(const string& tag, const string& attribute, double value, int which){
    char valueStr[255];
    sprintf(valueStr, "%lf", value);
    int tagID = writeAttribute(tag, attribute, valueStr, which) -1;
    return tagID;
...
//---------------------------------------------------------
int xmlFile::setAttribute(const string& tag, const string& attribute, const string& value, int which){
    int tagID = writeAttribute(tag, attribute, value, which) -1;
    return tagID;
}

//---------------------------------------------------------
TiXmlElement* xmlFile::getElementForAttribute(const string& tag, int which){
    vector<string> tokens = tokenize(tag,":");
    TiXmlHandle tagHandle = storedHandle;
    for (uint x = 0; x < tokens.size(); x++) {
        if (x == 0)
...
//---------------------------------------------------------
bool xmlFile::readIntAttribute(const string& tag, const string& attribute, int& outValue, int which){
...
//---------------------------------------------------------
bool xmlFile::readDoubleAttribute(const string& tag, const string& attribute, double& outValue, int which){
...
//---------------------------------------------------------
bool xmlFile::readStringAttribute(const string& tag, const string& attribute, string& outValue, int which){
...
//---------------------------------------------------------
int xmlFile::writeAttribute(const string& tag, const string& attribute, const string& valueString, int which){
    vector<string> tokens = tokenize(tag,":");
    TiXmlHandle tagHandle = storedHandle;
    for (uint x = 0; x < tokens.size(); x++) {
        if (x == 0)
...
//---------------------------------------------------------
bool xmlFile::loadFromBuffer(const string& buffer )
{
...
//---------------------------------------------------------
void xmlFile::copyXmlToString(string & str)
{
